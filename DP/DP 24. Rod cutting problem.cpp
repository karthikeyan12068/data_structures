...
ll ret(ll n,ll acap,vp_ll &v1,mp_ll &m1){
    if(n==0){
        if(v1[n].first>acap){
            return 0;
        }
        ll div=acap/v1[n].first;
        return v1[n].second*div;
    }
    ll inc=INT_MIN;
    if(m1[{n,acap}]!=0){
        return m1[{n,acap}];
    ...
    if(acap>=v1[n].first){
        inc=v1[n].second+ret(n,acap-v1[n].first,v1,m1);
    }
    ll ninc=0+ret(n-1,acap,v1,m1);
    return m1[{n,acap}]=max(inc,ninc);
}
ll ret(ll n,ll t,v_ll &v1,mp_ll &m1){
    if(n==0){
        ll div=t/v1[n];
        return div*v1[n];
    }
    ...
    ll inc=INT_MIN;
    if(t-(n+1)>=0) {
        inc = v1[n] + ret(n, t - (n+1), v1,m1);
    }
    ll ninc=0+ret(n-1,t,v1,m1);
    return m1[{n,t}]=max(inc,ninc);
}
int main() {
