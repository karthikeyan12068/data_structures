...
ll repdp(ll n,ll m,vector<v_ll>v1,vector<v_ll>dp){
    if(dp[n][m]!=-1){
        return dp[n][m];
    }
    ll left=INT_MAX,right=INT_MAX;
    if(n-1>=0) {
        left = v1[n][m] + repdp(n - 1, m, v1, dp);
    }
    if(m-1>=0) {
        right = v1[n][m] + repdp(n, m - 1, v1, dp);
    }
