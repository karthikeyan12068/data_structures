...
ll ret(ll n,ll m,vector<vector<ll>>&v1,mp_ll &m1){
    if(n==0 && m==0){
        return m1[{0,0}]=v1[0][0];
    }
    ...
    if(n-1>=0) {
        right=v1[n][m] + ret(n - 1, m, v1, m1);
    }
    if(m-1>=0) {
        down = v1[n][m] + ret(n, m - 1, v1, m1);
    }
    return m1[{n,m}]=min(right,down);
}
int main() {
