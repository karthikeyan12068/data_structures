...
ll ret(ll row,ll col,vector<vector<ll>>&v1,mp_ll &m1){
    if(row==v1.size() || col==v1[row].size() || col==-1){
        return 0;
    }
    if(m1[{row,col}]!=0){
        return m1[{row,col}];
    }
    if(row==0 && col!=v1[row].size()-1) {
        ll right=0+ret(row,col+1,v1,m1);
        ll down = v1[row][col] + ret(row + 1, col, v1, m1);
        ll diagright = v1[row][col] + ret(row + 1, col + 1, v1, m1);
        ll diagleft = v1[row][col] + ret(row + 1, col - 1, v1, m1);
        return m1[{row, col}] = max4(down, diagleft, diagright,right);
    }
    else {
        ll down = v1[row][col] + ret(row + 1, col, v1, m1);
        ll diagright = v1[row][col] + ret(row + 1, col + 1, v1, m1);
        ll diagleft = v1[row][col] + ret(row + 1, col - 1, v1, m1);
        return m1[{row, col}] = max3(down, diagleft, diagright);
    }
}
int main() {
